...
class CloseRegion : TileRegion() {
    override fun drawPath() {
        path.elements.clear()
        if (roundedCorner) {
            path.elements.add(MoveTo(size * 0.23809524, 0.0))
            path.elements.add(LineTo(size, 0.0))
            path.elements.add(LineTo(0.0, size))
            path.elements.add(LineTo(0.0, size * 0.23809524))
            path.elements.add(QuadCurveTo(0.0, 0.0, size * 0.23809524, 0.0))
