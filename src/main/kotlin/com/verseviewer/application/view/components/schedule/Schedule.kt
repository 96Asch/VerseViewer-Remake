package com.verseviewer.application.view.components.schedule
...
class Schedule : Fragment("My View") {
    ...
    private val groupItem = MenuItem("Link").apply {
        graphic = Styles.fontAwesome.create(FontAwesome.Glyph.LINK).apply {
            setColor(Color.GRAY)
        }
        setOnAction {
    ...
    private val ungroupItem = MenuItem("Unlink").apply {
        graphic = Styles.fontAwesome.create(FontAwesome.Glyph.UNLINK).apply {
            setColor(Color.GRAY)
        }
        setOnAction {
    ...
    private val deleteItem = MenuItem("Remove").apply {
        graphic = Styles.fontAwesome.create(FontAwesome.Glyph.TRASH).apply {
            setColor(Color.GRAY)
        }
        setOnAction {
    ...
    override val root = notificationPane {
        content = hbox {
            tv = tableview(controller.list) {
                ...
                addEventFilter(MouseEvent.MOUSE_DRAGGED, ::mouseReleased)
            ...
            vbox {
                ...
                togglebutton(selectFirst = false) {
                    graphic = Styles.fontAwesome.create(FontAwesome.Glyph.CIRCLE).apply {
                        setColor(Color.RED)
                    }
                    tooltip {
                        text = "Arm schedule"
                ...
                button {
                    graphic = Styles.fontAwesome.create(FontAwesome.Glyph.FOLDER_OPEN)
                    action { loadSchedule() }
                    tooltip {
                        text = "Open schedule"
                    }
    ...
    private fun mouseReleased(evt : MouseEvent) {
        dragVerseController.isShiftDown = evt.isShiftDown
    }
 }
